...
def teardown_module(module):
    p = Path(TEST_DB)
    if p.exists():
        # Windows can retain file locks briefly; retry a couple times
        for _ in range(3):
            try:
                p.unlink()
                break
...
def test_material_recalc_updates_changed_rate():
    ...
    with _open() as conn:
        price1 = conn.execute("SELECT unit_price FROM sign_types WHERE name='TestSign'").fetchone()[0]
    assert price1 == 100.0
    # Update material rate
    with _open() as conn:
        conn.execute("UPDATE material_pricing SET price_per_sq_ft=7.5 WHERE material_name='Aluminum'")
        conn.commit()
    dbm.recalc_prices_from_materials()
    with _open() as conn:
        price2 = conn.execute("SELECT unit_price FROM sign_types WHERE name='TestSign'").fetchone()[0]
